...
export const intersectionObserverConfigValidator = (config: TIntersectionObserverConfig) => {
  config = { ...testsPayload.intersectionObserverConfig, ...config };
  const keys: (keyof TIntersectionObserverConfig)[] = [`root`, `rootMargin`, `threshold`];
  const configKeys = (Object.keys(config) as (keyof TIntersectionObserverConfig)[]);
